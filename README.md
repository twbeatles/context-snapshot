...
3. 예: `30` 입력 시 30분마다 자동 저장

**Git 변경 감지:**
1. 설정 → General 탭
2. **Git 변경 시 자동 스냅샷** 체크
3. Git 상태(브랜치, 커밋 등) 변경 시 자동 저장
...
| 파일/폴더 | 설명 |
|-----------|------|
| `ctxsnap_win.py` | 메인 애플리케이션 (UI + 로직) |
| `ctxsnap/` | 핵심 패키지 |
| `ctxsnap/utils.py` | 유틸리티 함수 |
| `ctxsnap/storage.py` | 기본 스토리지 |
| `ctxsnap/app_storage.py` | 고급 스토리지 |
| `assets/` | 아이콘/이미지 |
