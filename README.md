...
- **검색/필터**: 제목/루트/태그 + 메모/TODO/최근 파일/앱까지 검색, 최근 N일/태그 복수 선택/정렬 옵션
- **최근 파일 스캔 제어**: 제외 폴더/스캔 제한/백그라운드 수집 옵션
- **복원 체크리스트/히스토리**: 복원 완료 후 TODO 체크리스트, `restore_history.json` 기록
- **자동 스냅샷**: 주기적 자동 저장 또는 Git 변경 감지 기반
- **대규모 스냅샷 대응**: 가상 리스트(virtualized list)로 목록 렌더링 최적화
